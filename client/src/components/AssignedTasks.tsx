...
export default function AssignedTasks() {
  ...
  const closedTasks = sortByRecentlyClosed(assignedTasks.filter(t => (t.status === 'done' || isParentWithAllMySubtasksDone(t)) && !t.parentTaskId));
    ...
    <div ref={containerRef} className="space-y-4" data-testid="assigned-tasks-section">
      ...
      <div className="flex items-center gap-3 rounded-md border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-amber-700 dark:text-amber-400">
        ...
        <div className="text-sm leading-relaxed">
          <p>Please add your end-of-shift reports or the work you did on the task as notes <StickyNote className="h-4 w-4 inline-block align-text-bottom mx-0.5" /> before closing a task.</p>
          <p>All due dates and times are displayed in your local date and time.</p>
        </div>
      </div>
